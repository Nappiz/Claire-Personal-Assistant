...
from app.observability.ai_telemetry import set_usage_context as telemetry_set_usage_context, reset_usage_context as telemetry_reset_usage_context
...
def set_usage_context(**values):
    ...
    return telemetry_set_usage_context(**values), correlation_token


def reset_usage_context(token):
    usage_token, correlation_token = token
    try:
        telemetry_reset_usage_context(usage_token)
    finally:
